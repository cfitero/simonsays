...
'''
After cpu sequence is repeated the player must attempt to copy the same pattern sequence. THe player is given 3 seconds to select a color and checks if the selected color matches the cpu pattern sequence. If player is unable to select a color within 3 seconds then the game is over and the pygame window closes.
'''
        ...
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONUP and event.button == 1:
                ...
                if green.selected(pos):
                    ...
                    check_sequence(players_sequence) #check if players choice was correct
...
'''
Checks if player's move matches the cpu patter sequence
'''
...
while True:
    ...
    draw_board() #drwas button onto pygame screen
    repeat_cpu_sequence() # repeates cpu sequence if it's not empty
